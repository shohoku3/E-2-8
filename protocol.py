...
class protocol(object):
    def __init__(self, NumPlayers, Nmaxones, PlayerInputSize, SecParam, bitLength, p, a, disableChecks, inputSet,keyWords):
        self.players = []
        self.params = pm.Paramaters(NumPlayers, Nmaxones, PlayerInputSize, SecParam, bitLength, p, a, disableChecks)
        self.create_Players(inputSet,keyWords)
        self.hashes = hashes.new(self.params.k)
        self.sumVals = []
    
    def create_Players(self,inputSet,keyWords):
        p0 = players.PlayerHub(0, self.params, inputSet)
        p1 = players.PlayerHub(1, self.params, keyWords)
    ...
    def perform_Output(self):
        ...
        return forPrint, intersections,output
